...
## [Outposts] Announcing availability of AWS Outposts racks in Rwanda
...
AWS Outposts racksがルワンダで利用可能になりました。ルワンダ国内のデータセンターやコロケーションにラック単位で出荷・設置でき、AWSのインフラ・サービス・APIをオンプレミスで利用できます。

### 変更内容・新機能の詳細

今回の発表により、AWS Outposts racksをルワンダのオンプレミス施設やコロケーションスペースへ配送・設置できるようになりました。Outposts racksはAWSのハードウェアを顧客サイトに配置し、AWSのインフラ、API、ツールをローカルで提供することで、クラウドと一貫したハイブリッド環境を実現します。低レイテンシでオンプレミスシステムにアクセスする必要があるワークロードをローカルで実行できるほか、データ主権やレジデンシ要件に従ってデータをオンプレミスで管理・処理する用途にも適しています。Outposts rackは“ホームリージョン”（管理用のAWSリージョン）に接続してコントロールプレーンや管理を行う設計のため、対応するAWSリージョンとの接続やサービス互換性（ローカルで利用可能なAWSサービスのサブセット）を事前に確認する必要があります。詳細な注文・設置手順やサポート国・地域の最新情報は製品概要、ユーザーガイド、Outposts rack FAQsを参照してください。

### 影響範囲・利用シーン

- **対象ユーザー**: エンタープライズ、規制業界（金融・医療・公共）、SaaS事業者、通信事業者、ハイブリッドアーキテクト
- **利用シーンまたは効果**: 低レイテンシが求められるアプリケーションのローカル実行、データ主権の確保が必要なデータ処理、オンプレとクラウドを統合した運用や段階的クラウド移行
- **運用効果**: AWSのAPI/ツールと一貫した運用が可能になり、オンプレミス側の遅延削減、コンプライアンス対応の容易化、中央リージョンによる一元管理で運用負荷を低減

### 技術的な注意点

- **IAM権限**: Outpostsの作成・管理には適切なAWSアカウントとIAM権限（サービスロール含む）が必要です。事前に権限を確認してください
- **リージョン制限**: Outposts rackは“ホームリージョン”への接続が必須です。ルワンダでのサポート開始は発表済みですが、利用可能なホームリージョンや国／地域の最新リストはOutposts rack FAQsで確認してください
- **コスト**: ハードウェア購入/リース、設置・配送費、運用サポートの料金が発生します。ネットワーク（Direct Connect等）やデータ転送にも追加コストがかかる可能性があります
- **ネットワーク要件**: 管理プレーンやサービス連携のためにホームリージョンへの安定した接続（帯域・レイテンシ要件）が必要です。Direct Connectの利用が推奨されるケースがあります
- **設置要件**: 物理的なラックスペース、電源（冗長化含む）、冷却、現地設置作業のためのスペース確保が必要です。設置はAWSまたは認定パートナーが行います
- **サービス互換性**: 全てのAWSサービスがローカルで利用できるわけではありません。Outposts上でサポートされるサービスとAPIの一覧をユーザーガイドで確認してください

### 参考情報

- https://aws.amazon.com/about-aws/whats-new/2026/01/availability-aws-outposts-racks-rwanda
