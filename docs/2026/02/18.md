...
今回の機能追加では、AWS Glueのカタログフェデレーション機能を経由して、外部（リモート）のApache Iceberg RESTカタログをAWS Clean Roomsのコラボレーションで参照できるようになりました。技術的にはGlueがフェデレーションレイヤーとして機能し、Iceberg RESTカタログに格納されるメタデータをそのまま参照することで、対象データ（Icebergテーブル）は引き続きS3上に保持され、Clean Rooms側でメタデータの複製やETLを行う必要がありません。これにより、データ所有者は基データを共有せずに、コラボレーション参加者と集計や照合などの分析を行えます。
主なポイント:
...
- https://docs.aws.amazon.com/cleanrooms/latest/userguide/
- https://docs.aws.amazon.com/glue/latest/dg/catalog-federation.html
...
- https://aws.amazon.com/about-aws/whats-new/2026/02/amazon-opensearch-service-expands-support-graviton4-based-instances
...
- https://aws.amazon.com/about-aws/whats-new/2026/02/amazon-opensearch-service-supports-i7i-instances
- https://aws.amazon.com/opensearch-service/
- https://aws.amazon.com/opensearch-service/pricing/
