...
- Word拡張（更新）: Wordのプリミティブ（書式/スタイル）を用いたフォーマット済みドキュメント生成、トラックチェンジを有効にした一括編集（redlining）、コメントレビューへの参加（レビュワーとしての操作）が可能になり、契約レビューや長文編集の効率が向上します。
これらはプレビュー機能であり、まずサポートされるリージョン（US East/N. Virginia 等）で利用可能です。利用にはAmazon Quickアカウントの登録と拡張機能のインストールが必要です。
